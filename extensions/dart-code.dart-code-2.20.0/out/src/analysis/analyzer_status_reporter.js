...
class AnalyzerStatusReporter {
        ...
        if (this.analysisInProgress) {
            ...
            setTimeout(() => {
                ...
                if (this.analysisInProgress && !this.analyzingPromise) {
                    vscode_1.window.withProgress({ location: vscode_1.ProgressLocation.Window, title: "Analyzing…" }, (_) => {
    ...
    handleServerError(error, method) {
        ...
        log_1.logError(error.message, utils_1.LogCategory.Analyzer);
        if (error.stackTrace)
            log_1.logError(error.stackTrace, utils_1.LogCategory.Analyzer);
...
`;
        const fileName = `bug-${utils_2.getRandomInt(0x1000, 0x10000).toString(16)}.md`;
